...
let chartButton (chart: Chart) =
    let baseCssCls = "chartbutton hover:bg-gray-700 border p-1 m-1 cursor-pointer text-xs w-32 h-12"
    let chartCssCls =
...
let pdfViewer =
    ...
    object [ _id "pdfViewer"; _class baseCssCls; _type "application/pdf"; _width "100%"; _height "100%"; __data "" ] []

let renderPage (viewModel: (Airport * Chart list) seq) =
    
    let navContentSection =
        viewModel
        |> Seq.map fst
        |> navBar
    ...
    html [ _class "font-mono bg-gray-800 h-full" ] [
        head [] [
            title [] [ str "Charts" ]
            link [ _rel "stylesheet"; _href "style.css" ]
            script [ _src "script.js" ] []
        ]
        body [_class "text-gray-300 h-full"] [
            div [] [ navContentSection ]
            div [ _class "flex flex-row h-full" ] [
